...
struct ValueObservationSubscriber<Value: Sendable>: Sendable {
  ...
  func receive(
    _ outcome: Result<ValueObservationChange<Value>, any Error>,
...
///
/// See ``ValueObservationSubscriberRegistry/publish(_:)`` for why they travel with the outcome.
struct ValueObservationPublication<Value: Sendable>: Sendable {
  let outcome: Result<ValueObservationChange<Value>, any Error>
  let subscribers: [ValueObservationSubscriber<Value>]
}

/// A read a runtime has issued, and the revision the observation was at when it was issued.
struct ValueObservationReadRequest: Sendable {
  let revision: UInt64
...
struct ValueObservationReadCoordinator: Sendable {
  ...
  mutating func requireInitialRead() -> ValueObservationReadRequest? {
    guard
  ...
  mutating func requireRead(source: ValueObservationSource) -> ValueObservationReadRequest? {
    self.revision &+= 1
    self.readIsRequired = true
  ...
  mutating func completeRead(_ request: ValueObservationReadRequest) -> Bool {
  ...
  mutating func takeRequestIfPossible() -> ValueObservationReadRequest? {
    guard self.readIsRequired, !self.readIsInFlight else { return nil }
    self.readIsRequired = false
    self.readIsInFlight = true
    return ValueObservationReadRequest(revision: self.revision, source: self.requiredSource)
  }
}

/// The subscribers attached to one value observation runtime, and the value a late one is caught
/// up with.
struct ValueObservationSubscriberRegistry<Value: Sendable>: Sendable {
...
struct ValueObservationDeliveryQueue<Value: Sendable>: Sendable {
  private var publications = [ValueObservationPublication<Value>]()
  private var isDelivering = false

  /// Queues `publication`, reporting whether the caller takes on draining the queue.
  mutating func enqueue(_ publication: ValueObservationPublication<Value>) -> Bool {
  ...
  mutating func next() -> ValueObservationPublication<Value>? {
