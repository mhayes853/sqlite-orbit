...
/// }
/// ```
public protocol OrbitDatabaseRowCursor: ~Copyable, ~Escapable {
...
/// }
/// ```
public protocol OrbitDatabaseCursor<Element>: ~Copyable, ~Escapable {
...
extension OrbitDatabaseCursor where Self: ~Copyable, Self: ~Escapable {
  /// Calls `body` for every remaining value in the cursor.
  ///
  /// - Parameter body: Receives each value in turn.
  /// - Throws: Whatever `body` throws, or whatever ``next()`` throws.
...
///
/// Created by ``OrbitDatabaseCursor/map(_:)``.
///
/// ```swift
/// var titles = try transaction.fetchCursor(Reminder.all).map(\.title)
/// ```
public struct OrbitDatabaseMapCursor<Base: OrbitDatabaseCursor, Output>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
...
///
/// Created by ``OrbitDatabaseCursor/filter(_:)``.
///
/// ```swift
/// var pending = try transaction.fetchCursor(Reminder.all).filter { !$0.isCompleted }
/// ```
public struct OrbitDatabaseFilterCursor<Base: OrbitDatabaseCursor>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
...
///
/// Created by ``OrbitDatabaseCursor/compactMap(_:)``.
///
/// ```swift
/// var ids = try transaction.fetchCursor(Reminder.all).compactMap { Int($0.title) }
/// ```
public struct OrbitDatabaseCompactMapCursor<Base: OrbitDatabaseCursor, Output>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
  public typealias Element = Output
...
///
/// Created by ``OrbitDatabaseCursor/dropFirst(_:)``.
///
/// ```swift
/// var afterTheFirstTen = try transaction.fetchCursor(Reminder.all).dropFirst(10)
/// ```
public struct OrbitDatabaseDropFirstCursor<Base: OrbitDatabaseCursor>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
  public typealias Element = Base.Element
...
///
/// Created by ``OrbitDatabaseCursor/drop(while:)``.
///
/// ```swift
/// var fromTheFirstPending = try transaction.fetchCursor(Reminder.all)
///   .drop(while: \.isCompleted)
/// ```
public struct OrbitDatabaseDropWhileCursor<Base: OrbitDatabaseCursor>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
  public typealias Element = Base.Element
...
///
/// Created by ``OrbitDatabaseCursor/prefix(_:)``.
///
/// ```swift
/// var firstTen = try transaction.fetchCursor(Reminder.all).prefix(10)
/// ```
public struct OrbitDatabasePrefixCursor<Base: OrbitDatabaseCursor>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
  public typealias Element = Base.Element
...
///
/// Created by ``OrbitDatabaseCursor/prefix(while:)``.
///
/// ```swift
/// var leadingCompleted = try transaction.fetchCursor(Reminder.all)
///   .prefix(while: \.isCompleted)
/// ```
public struct OrbitDatabasePrefixWhileCursor<Base: OrbitDatabaseCursor>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
  public typealias Element = Base.Element
...
///
/// Created by ``OrbitDatabaseCursor/enumerated()``.
///
/// ```swift
...
/// }
/// ```
public struct OrbitDatabaseEnumeratedCursor<Base: OrbitDatabaseCursor>:
  OrbitDatabaseCursor, ~Copyable, ~Escapable
where Base: ~Copyable, Base: ~Escapable {
  /// The value this cursor produces for each row.
...
extension OrbitDatabaseCursor where Self: ~Copyable, Self: ~Escapable {
  ...
  ) -> OrbitDatabaseMapCursor<Self, Output> {
    OrbitDatabaseMapCursor(base: consume self, transform: transform)
  }
  ...
  ) -> OrbitDatabaseFilterCursor<Self> {
    OrbitDatabaseFilterCursor(base: consume self, predicate: predicate)
  }
  ...
  public consuming func compactMap<Output>(
    _ transform: @escaping (Element) throws -> Output?
  ) -> OrbitDatabaseCompactMapCursor<Self, Output> {
    OrbitDatabaseCompactMapCursor(base: consume self, transform: transform)
  }
  ...
  @_lifetime(copy self)
  @inlinable
  public consuming func dropFirst(_ count: Int) -> OrbitDatabaseDropFirstCursor<Self> {
    OrbitDatabaseDropFirstCursor(base: consume self, count: count)
  }
  ...
  ) -> OrbitDatabaseDropWhileCursor<Self> {
    OrbitDatabaseDropWhileCursor(base: consume self, predicate: predicate)
  }
  ...
  @_lifetime(copy self)
  @inlinable
  public consuming func prefix(_ maxLength: Int) -> OrbitDatabasePrefixCursor<Self> {
    OrbitDatabasePrefixCursor(base: consume self, count: maxLength)
  }
  ...
  public consuming func prefix(
    while predicate: @escaping (Element) throws -> Bool
  ) -> OrbitDatabasePrefixWhileCursor<Self> {
    OrbitDatabasePrefixWhileCursor(base: consume self, predicate: predicate)
  }
  ...
  @_lifetime(copy self)
  @inlinable
  public consuming func enumerated() -> OrbitDatabaseEnumeratedCursor<Self> {
    OrbitDatabaseEnumeratedCursor(base: consume self)
  }
  ...
  /// database when only the count is wanted; see ``OrbitDatabaseReadTransaction/fetchCount(_:)``.
  ///
  ...
  public consuming func topK(
    ...
    var heap = OrbitDatabaseCursorHeap<Element>(capacity: k)
    try forEach { value in
  ...
  public consuming func minMaxK(
    ...
    var minimums = OrbitDatabaseCursorHeap<Element>(capacity: k)
    var maximums = OrbitDatabaseCursorHeap<Element>(capacity: k)
    try forEach { value in
