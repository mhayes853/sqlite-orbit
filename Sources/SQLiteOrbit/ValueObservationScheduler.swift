...
///
/// Use one of the built-in schedulers — ``ValueObservationScheduler/immediate``,
/// ``ValueObservationScheduler/async(priority:)``, ``ValueObservationScheduler/async(on:priority:)``,
/// or ``ValueObservationScheduler/mainActor`` — unless you need delivery somewhere none of them
/// reaches.
///
/// ```swift
...
extension ValueObservationScheduler where Self == AsyncValueObservationScheduler {
  ...
  ///
  /// This is the scheduler ``ValueObservation/subscribe(to:isolation:onError:onChange:)`` uses.
  ///
  /// ```swift
...
/// ``ValueObservation/subscribe(to:scheduling:onError:onChange:)``, whose callbacks are
/// main-actor isolated.
///
/// ```swift
...
public struct MainActorValueObservationScheduler: ValueObservationMainActorScheduler {
  private let drain = ValueObservationSchedulerDrain(isolation: MainActor.shared, priority: nil)
