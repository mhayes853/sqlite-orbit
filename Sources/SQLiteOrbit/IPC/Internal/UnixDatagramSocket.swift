  ...
  struct UnixSocketAddress {
    ...
    init(path: String) throws {
      guard !path.utf8.contains(0) else {
        throw OrbitIPCSystemError(operation: "socket path contains NUL", code: EINVAL)
      }
  ...
  final class UnixDatagramSocket: Sendable {
    ...
    init(path: String, receiveBufferByteCount: Int) throws {
      ...
      do {
        ...
        guard result == 0 else {
          throw OrbitIPCSystemError(operation: "bind", code: errno)
        }
      } catch {
    ...
    func send(_ bytes: [UInt8], to path: String) throws -> Bool {
      try self.storage.withLock { storage in
        guard !storage.isClosed else {
          throw OrbitIPCSystemError(operation: "sendto", code: EBADF)
        }
        var address = try UnixSocketAddress(path: path)
    ...
    func receive(maximumByteCount: Int) throws -> [UInt8]? {
      try self.storage.withLock { storage in
        guard !storage.isClosed else {
          throw OrbitIPCSystemError(operation: "recv", code: EBADF)
        }
    ...
    private static func makeDescriptor() throws -> Int32 {
      ...
      guard descriptor >= 0 else {
        throw OrbitIPCSystemError(operation: "socket", code: errno)
      }
      return descriptor
    }
    ...
    private static func configure(
      ...
      guard flags >= 0, fcntl(descriptor, F_SETFL, flags | O_NONBLOCK) == 0 else {
        throw OrbitIPCSystemError(operation: "fcntl", code: errno)
      }
      ...
      guard result == 0 else {
        throw OrbitIPCSystemError(operation: "setsockopt", code: errno)
      }
    }
