...
@Suite
struct OrbitValueObservationSubscriberRegistryTests {
  private func subscriber(
    onChange: @escaping @Sendable (OrbitValueObservationChange<Int>) -> Void = { _ in }
  ...
  func aSubscriberRegisteringDuringAPublicationIsCaughtUpExactlyOnce() throws {
    ...
    let change = OrbitValueObservationChange(value: 1, source: .initial)
  ...
  func theFirstSubscriberIsTheOneThatStartsTheObservation() {
    ...
    _ = registry.publish(OrbitValueObservationChange(value: 1, source: .initial))
